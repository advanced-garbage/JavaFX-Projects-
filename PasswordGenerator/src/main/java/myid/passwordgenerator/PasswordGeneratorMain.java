    ...
    public void start(Stage stage) {
        ...
        generate.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent e) {
                ...
                boolean isTextFieldNum = true;
                char[] numCheck = new char[10];
                if (tf_set_length.getText().length() > 0) {
                    numCheck = tf_set_length.getText().toCharArray();
                }
                for (char c : numCheck) {
                    if (!Character.isDigit(c)) {
                        isTextFieldNum = false;
                    }
                }
                ...
                } else if (isTextFieldNum) {
                    passwordSB.append("Invalid input.");
                } else {
                    int length = Integer.parseInt(tf_set_length.getText().toString());
